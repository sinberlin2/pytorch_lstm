...
lr=0.0002
lr_decay=0.0
fut_pred=3 # how many predictions are made into the future
batch_size=64
sliding_window=10
...
def main():
    ...
    if load_model == False:
        ...
        if init_batch == True:
            ...
            model = pytorch_model.LSTM(batch_size=batch_size, input_size=input_size, seq_len=sliding_window,
                                       hidden_layer_size=hidden_layer_size,
                                       num_layers=num_layers, output_size=output_size,
                                       dropout=dropout)
    ...
    utils.plot_test_predictions(dataset, sliding_window, results_folder, u2_value, test_preds, pred_var, fut_pred, pred_index=0, show_last=False)
    # #plot last 50 predictions of first and last index
    utils.plot_test_predictions(dataset, sliding_window, results_folder, u2_value, test_preds, pred_var, fut_pred, pred_index= 0, show_last=10)
    utils.plot_test_predictions(dataset, sliding_window, results_folder, u2_value, test_preds, pred_var, fut_pred, pred_index= -1, show_last=20)
