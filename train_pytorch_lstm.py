...
def train(train_inout_seq, val_inout_seq, train_window, epochs, batch_size, input_size, hidden_layer_size, num_layers, output_size, lr, lr_decay, dropout, results_folder, stateful, stateful_batches):

    model = pytorch_lstm_model.LSTM(batch_size=batch_size, input_size=input_size, seq_len=train_window,
    ...
    if stateful==True:
        print('Every batch is initialised with zeros.')
    if stateful_batches==1:
        print('Epochs are initialised with zeros, batches are stateful. The hidden state is also passed on between the individual sequences of the batch.')
    else:
        print('Epochs are initialised with zeros, batches are stateful. Hidden states are passed on between batches.')

    #for (mini) batches, shuffle should be false since we are dealing with ordered time sequences
    train_loader = torch.utils.data.DataLoader(train_inout_seq, batch_size= batch_size, shuffle=False, num_workers=1, drop_last=True )
    ...
    for i in range(epochs):
        ...
        for step, (seq, labels) in enumerate(train_loader):
            ...
            if stateful==1:
                ...
                y_pred, model.hidden_cell = model.forward(seq, (model.hidden_cell[0].detach(), model.hidden_cell[1].detach()), print_hidden=False, stateful_batches=stateful_batches)
        ...
        for val_batch_no, (seq, labels) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                if stateful==1:
                    ...
                    y_pred, _ = model.forward(seq, stateful_batches=stateful_batches)
