...
def test_model(model, test_inout_seq, input_size, fut_pred, tw, stateful, stateful_batches):
	...
	for seq_no, (seq, labels) in enumerate(test_loader):
		...
		for pred_no in range(fut_pred):
			if seq_no + pred_no < len_preds:  # to make sure that we can get a label for the next prediciton step
				...
				with torch.no_grad():  # reduces time cause no gradients are being calculated
					if pred_no == 0:
						...
						test_prediction, (h, c) = model.forward(seq, model.hidden_cell,print_hidden=False, stateful_batches= stateful_batches) #not sure if stateful_batches is needed here
					else:
					...
					naive_truth = seq[:, -1, :]
					naive_predictions_all[seq_no, pred_no] = naive_truth
					...
					# record test prediction for each sequence
					test_predictions_all[seq_no, pred_no ,:] = test_prediction

					# add the next label value to labels with multiple predictions (change maybe to use labels1)
					if pred_no > 0:
						add_to_label = torch.DoubleTensor(test_inout_seq[seq_no + pred_no][1])
